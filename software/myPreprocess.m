classdef myPreprocess
    ...
    methods
        function obj=myPreprocess(dirc,method,constraint,varargin)
            obj.tech=method;
            obj.cons=constraint;
            if ~isempty(varargin)
            ...
            if obj.islocal==1
                if strcmp(obj.cons,'spf')==1
                    for i=1:length(imds.Files)
                        ...
                        elseif strcmp(str_name,'lv_theta_d')==1
                            obj.sup.lv_theta_d=readgeoraster(imds.Files{i});
                        end
                        clear str_name
                    end
                elseif strcmp(obj.cons,'apf')==1
                    for i=1:length(imds.Files)
                        str_name=extractAfter(imds.Files{i},str1);
                        str_name=extractBefore(str_name,str2);
                        if strcmp(str_name,'aspect_1000')==1
                            obj.sup.aspect=readgeoraster(imds.Files{i});
                        elseif strcmp(str_name,'lv_phi_a')==1
                            obj.sup.lv_phi_a=readgeoraster(imds.Files{i});
                        elseif strcmp(str_name,'lv_phi_d')==1
                            obj.sup.lv_phi_d=readgeoraster(imds.Files{i});
                ...
                if strcmp(obj.cons,'apf')==1
                    for i=1:length(imds.Files)
                        str_name=extractAfter(imds.Files{i},str1);
                        str_name=extractBefore(str_name,str2);
                        if strcmp(str_name,'aspect')==1
                            obj.sup.aspect=readgeoraster(imds.Files{i});
                        end
                        clear str_name
                    end
                elseif strcmp(obj.cons,'spf')==1
                    for i=1:length(imds.Files)
                        str_name=extractAfter(imds.Files{i},str1);
                        str_name=extractBefore(str_name,str2);
                        if strcmp(str_name,'dem_ddew')==1
